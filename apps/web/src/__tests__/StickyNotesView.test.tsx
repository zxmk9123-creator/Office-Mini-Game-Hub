...
describe("StickyNotesView", () => {
  ...
  it("plain text: click the textarea, type ordinary text, see it immediately, and persist it on blur", async () => {
    mockedListStickyNotes.mockResolvedValue([{ ...NOTE_A, content: "" }]);
    mockedUpdateStickyNote.mockResolvedValue({ ...NOTE_A, content: "TEST" });
    ...
    fireEvent.blur(textarea);
    await waitFor(() => expect(mockedUpdateStickyNote).toHaveBeenCalledWith("s1", "player-1", { content: "TEST" }));
  });
